...
u8 exfuns_init(void)
{
	fs[0]=(FATFS*)pvPortMalloc(sizeof(FATFS));	//为磁盘0工作区申请内存	
	fs[1]=(FATFS*)pvPortMalloc(sizeof(FATFS));	//为磁盘1工作区申请内存
...
//#endif	  
		*total=(float)tot_sect*512/1024;	//KB
		*free=(float)fre_sect*512/1024;	//KB
 	}
	return res;
}		   
