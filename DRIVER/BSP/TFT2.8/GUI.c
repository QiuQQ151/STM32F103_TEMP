...
#include "lcd.h"         //调用LCD操作库
#include "fontupdate.h"  //调用外部GBK汉字字库
#include "FONT.h"    //内部asc2库
...
//画圆
void Draw_Circle(int xc, int yc,u16 c,int r, int fill)
{
	int x = 0, y = r, yi, d;
...
void Draw_Fill_Triangel(u16 x0,u16 y0,u16 x1,u16 y1,u16 x2,u16 y2)
{
  ...
  if (y0 > y1) 
		...
		else if(x2 > b)
    {
		...
		Draw_Fill_Rectangle(a,y0,b,y0,POINT_COLOR);
    return;
	}
	dx01 = x1 - x0;
	dy01 = y1 - y0;
    ...
    if(a > b)
    {
		...
		Draw_Fill_Rectangle(a,y,b,y,POINT_COLOR);
	}
	sa = dx12 * (y - y1);
	sb = dx02 * (y - y0);
	for(; y<=y2; y++) 
...
void Draw_Button(u16 x,u16 y,u16 color,u8 strcount,u8 fontsize)
{
	 POINT_COLOR=BLACK;
	 Draw_Line(x-1,y-1,x+strcount*8+1,y-1);
	 Draw_Line(x-1,y-1,x-1,y+fontsize+1);
	 Draw_Line(x-1,y+fontsize+1,x+strcount*8+1,y+fontsize+1);
	 Draw_Line(x+strcount*8+1,y-1,x+strcount*8+1,y+fontsize+1);
	 POINT_COLOR=GRAY;
	 Draw_Line(x-1,y+fontsize+2,x+strcount*8+2,y+fontsize+2);
	 Draw_Line(x+strcount*8+2,y-1,x+strcount*8+2,y+fontsize+2);
	 Draw_Fill_Rectangle(x,y,x+strcount*8,y+fontsize,color);
}

void ShowFontStrButton(u16 x,u16 y,u16 fc,u16 bc,u8* str,u8 size,u16 button_color)
{
	Draw_Button(x,y,button_color,strlen(str),size);
	ShowFontStr(x,y,fc,bc,str,size,1);
}
...
/*   字符显示部分    */
//单个字符显示
extern _font_info ftinfo; //外部字库信息
...
////显示字符串
//void ShowString(u16 x,u16 y,u16 fc, u16 bc,u8 *p,u8 size,u8 mode)
//{         
//    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
//    {   
//		    if(x>(lcddev.width-1)||y>(lcddev.height-1))return; 	    
//        ShowChar(x,y,fc,bc,*p,size,mode);
//        x+=size/2;
//        p++;
//    }  
//} 
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				ShowChar(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,' ',size,0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	ShowChar(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,temp+'0',size,0); 
	}
} 


//显示一个指定大小的汉字
//x,y :汉字的坐标 font:汉字GBK码  size:字体大小  mode:0,正常显示,1,叠加显示   
void ShowFont(u16 x,u16 y,u16 fc, u16 bc,u8 *font,u8 size,u8 mode)
{
	u8 temp,t,t1;
	u16 y0=y;
	u8 dzk[72];  
	...
	{   												   
		...
		for(t1=0;t1<8;t1++)//绘制
		{
			if(temp&0x80)Draw_Point(x,y,fc);
			else if(mode==0)Draw_Point(x,y,bc); 
			temp<<=1;
			y++;
			if((y-y0)==size)
			{
...
//size :字体大小
//mode:0,非叠加方式;1,叠加方式    	   		   
void ShowFontStr(u16 x,u16 y,u16 fc,u16 bc,u8* str,u8 size,u8 mode)
{					
	  u16 x0=x;
	  u16 y0=y;			
    u8 UNItoGBK[2];	
    ...
    { 
        if(!bHz)
        {
	        ...
	        {   
		  			   ...
		  			   ShowChar(x,y,fc,bc,*str,size,mode);//有效部分写入 不转换时使用
//						  //转成GBK编码 字库里的太丑，不用，占双字节，浪费屏幕空间
//              UNItoGBK[0]=(*str+41856)>>8;
//						  UNItoGBK[1]=(*str+41856)&0x00FF;
//						  ShowFont(x,y,fc,bc,UNItoGBK,size,mode);
						  str++; 
		          x+=size/2; //字符,为全字的一半 
	        }
        }
        ...
        {     
            bHz=0;//有汉字库    
            if(ftinfo.fontok!=0xAA)//字库丢失
						{  ShowChar(x,y,fc,bc,'x',size,mode); }
						else 
						{  ShowFont(x,y,fc,bc,str,size,mode); }//显示这个汉字,空心显示 
	           str+=2; 
	           x+=size;//下一个汉字偏移	    
        }						 
    }   
...
//size :字体大小
//mode:0,非叠加方式;1,叠加方式    	   		   
void ShowFontStr_area(u16 x,u16 y,u16 width,u16 height,u8* str,u8 size,u8 mode)
{					
	  u16 x0=x;
	  u16 y0=y;							  	  
    u8 bHz=0;     //字符或者中文  	 
    if(mode==0)
			{ 
				Draw_Fill_Rectangle(x,y,x+width,y+height,WHITE);
			}			
    while(*str!=0)//数据未结束
    { 
        if(!bHz)
             ...
             if(x>(x0+width-size/2))//换行
		         ...
		         {         
		                ...
		                str++; 
						 ...
						 { ShowChar(x,y,0x0000,0xFFFF,*str,size,mode); }//有效部分写入 
        ...
        {     
            ...
            if(x>(x0+width-size))//换行
	           ...
	           if(y>(y0+height-size))break;//越界返回  						     
						 ...
						 if(ftinfo.fontok!=0xAA)//字库丢失
						{   ShowChar(x,y,0x0000,0xFFFF,'x',size,mode); }
						 else 
						{   ShowFont(x,y,0x0000,0xFFFF,str,size,mode); }//显示这个汉字,空心显示 
	           str+=2; 
	           x+=size;//下一个汉字偏移	    
        }						 
    }   
 ...
 * @parameters :x:the bebinning x coordinate of the Character display position
                y:the bebinning y coordinate of the Character display position
								...
								num:the asc2 code of display character(0~94)
